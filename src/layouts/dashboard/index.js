      ...
      <Box p={2} sx={{ backgroundColor: theme.palette.background.paper, boxShadow: "0px 0px 2px rgba(0,0,0,0.25)", height: '100vh', width: 100 }}>
        <Stack direction='column' alignItems='center' justifyContent='space-between' sx={{ height: '100%' }} spacing={3}>
          <Stack alignItems='center' spacing={4}>
            <Box sx={{ background: theme.palette.primary.main, height: 64, width: 64, borderRadius: 1.5 }}>
            ...
            <Stack sx={{ width: 'max-content' }} direction='column' alignItems='center' spacing={3}>
              {Nav_Buttons.map((el) =>
                el.index === selected ?
                  ...
                  <IconButton onClick={() => setSelected(el.index)} key={el.index} sx={{ width: 'max-content', color: theme.palette.mode === "light" ? '#000' : theme.palette.text.primary }}>
                    {el.icon}
                  </IconButton>
              )}
